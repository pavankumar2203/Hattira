...
module.exports = function(Events, app, auth) {
  ...
  app.route('/nearevents/:lat/:lng')
    .get(groupevents.nearbyevents);

  app.param('eventId', groupevents.event);

  app.route('/album/:eventID')
    .post(groupevents.postImage);

  app.route('/eventimage/:eventID')
    .post(groupevents.posteventImage);
};
